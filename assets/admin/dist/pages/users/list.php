<div id="layoutSidenav_content">
  <div class="container-fluid px-4" style="overflow-x: scroll;">
    ...
    <table class="table">
      <a class="btn btn-success float-end me-2 <?php echo $css; ?>" href="./index.php?pages=users&action=add"><i class="fa fa-plus"></i> Thêm mới</a>
      <thead>
        <tr>
          ...
          <th scope="col" class="text-black align-middle pt-3 pb-3 font-weight-bold">Trạng thái</th>
      ...
      <?php foreach (getAllUser() as $item) : ?>
        ...
        <tr>
          ...
          <td class="font-weight-bold align-middle truncate-text"><?= $item['email'] ?></td>
          ...
          <td class="font-weight-bold align-middle"><?= $item['status'] ?></td>
          <td class="font-weight-bold align-middle"><?= $item['create_at'] ?></td>
          <td colspan="2" class="font-weight-bold align-middle">
            <a class="btn btn-danger mb-2 <?php echo $css; ?>" href="./index.php?pages=users&action=edit&edit=<?= $item['id'] ?>"><i class="fa fa-edit"></i></a>
            <a href="./index.php?pages=users&action=list&delete=<?= $item['id']; ?>" class="btn btn-danger mb-2 <?php echo $css; ?>" onclick="return confirm('Bạn có chắc chắn muốn xóa?');"><i class="fa fa-trash"></i></a>
          </td>
        </tr>
      <?php endforeach ?>
    </table>
